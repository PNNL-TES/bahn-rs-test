...
Answer: Extremely well! The relationship between predicted SR (via SR at MAT) and reported annual SR is 1:1, with a slope=1 (P=`r round( slope_test, 3 )`) and intercept=0 (P=`r round( intercept_test, 3 )`). There is significant unexplained variability, with R^2^=`r round( summary( m )$r.squared, 2 )` and RSE of `r round( summary( m )$sigma, 0 )` g C m^-2^ yr^-1^.
