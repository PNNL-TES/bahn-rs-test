...
Rs_comparion_figure <- function( srdb, name="" ) {
	...
	p <- p + geom_errorbarh( aes( xmin=Rs_annual-Rs_spatial_err, xmax=Rs_annual+Rs_spatial_err), alpha=0.5 )
...
climate_figure <- function( srdb, name="" ) {
    ...
    p <- p + geom_jitter( data=srdb, aes( x=TAIR_LTM, y=PRECIP_LTM, color=Ecosystem_type ) ) #, show_guide=F )
    p <- p + guides( fill=F )
    print( p )
...
filtration3 <- function( srdb, tdiff_max=TDIFF_MAX, quiet=F ) {
	fields <- c( 	"Record_number", "Study_number",
					...
					"Rs_annual", "Rs_spatial_err", "Rs_wet", "Rs_dry", "RC_annual",
					"Model_type", "R10",
