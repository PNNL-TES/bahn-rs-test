...
compute_rs_mat <- function( s, Temp ) {
	...
	for( i in 1:nrow( s ) ) {
		...
		s[ i, "Rs_annual_bahn" ] <- if (biome == 'Mediterranean') { 436.2 * (rs_umol ^ 0.926) } else {
		  455.8 * ( rs_umol ^ 1.0054 ) }	
	} # for
	return( s )
} # compute_rs_mat
...
#************************************************************************************************
# update bahn model's parameter
compute_rs_new1 <- function( s, Temp ) {
	# Compute model responses at MAT, taking into account different output units
	# conversions <- read.csv( CONVERSIONS, stringsAsFactors=F )
	s$mtr_out <- F
	um <- 0
	...
	for( i in 1:nrow( s ) ) {
		...
		# seperate into mv mediterrean and keep mediterrean
		# update parameters for Bahn et al. function
		s[ i, "Rs_annual_bahn" ] <- if (biome == 'Mediterranean') { 672.4058 * (rs_umol ^ 0.53985) } else {
		  477.2924 * ( rs_umol ^ 0.85483 ) }	
	} # for
	return( s )
} # compute_rs_mat
...
#************************************************************************************************
# update bahn model's parameter with SPI and PDSI parameters
compute_rs_new2 <- function( s, Temp ) {
	# Compute model responses at MAT, taking into account different output units
	# conversions <- read.csv( CONVERSIONS, stringsAsFactors=F )
	s$mtr_out <- F
	um <- 0
	...
	for( i in 1:nrow( s ) ) {
		...
		# seperate into mv mediterrean and keep mediterrean
		# update parameters for Bahn et al. function
		s[ i, "Rs_annual_bahn" ] <- if (biome == 'Mediterranean') { 692.09113 * (rs_umol ^ 0.54927) + 108.58617*spi } else {
		  472.15683 * ( rs_umol ^ 0.86099 ) + 17.80091*spi + 12.16624*pdsi }	
	} # for
	return( s )
} # compute_rs_mat
...
#************************************************************************************************
# TAnnual as predictor with SPI and PDSI parameters
compute_rs_maat <- function( s, Temp ) {
	# Compute model responses at MAT, taking into account different output units
	# conversions <- read.csv( CONVERSIONS, stringsAsFactors=F )
	s$mtr_out <- F
	um <- 0
	...
	for( i in 1:nrow( s ) ) {
		...
		# seperate into mv mediterrean and keep mediterrean
		# update parameters for Bahn et al. function
		s[ i, "Rs_annual_bahn" ] <- if (biome == 'Mediterranean') { 729.09225 * (rs_umol ^ 0.46535) + 89.7789*spi } else {
		  588.618 * ( rs_umol ^ 0.65022 ) + 22.59026*spi + 11.29775*pdsi }	
	} # for
	return( s )
} # compute_rs_mat
