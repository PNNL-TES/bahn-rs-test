...
```{r, results='hide', message=FALSE, include=FALSE, echo=FALSE}
# load package
liblist <- c("ggplot2", "reshape", 'data.table', 'sqldf', "gridExtra",
             'tidyverse','dplyr','tidyr',"ggplot2", "lubridate", 'kableExtra', 'lattice', 'cowplot', 'drake', 'knitr')
...
```{r, results='hide', message=FALSE, include=TRUE, warning=FALSE, echo=FALSE, fig.height = 5, fig.width = 7}
figA <- climate_figure( srdb_orig )
```


## 3.2 test the relationship between Rs_annual and Rs_mat
```{r, results='show', message=TRUE, include=TRUE, echo=FALSE, fig.height = 5, fig.width = 7, echo=FALSE}
...
AC_test( srdb, srdb$Annual_TS_Coverage, 'TS_coverage', 'Effects of soil temperature coverage on Rs_Annual vs. Rs_annual_bahn' )
AC_test( srdb, srdb$Annual_coverage, 'Rs_coverage', 'Effects of air temperature coverage on Rs_Annual vs. Rs_annual_bahn' )

```

## 3.4.1 Test whether outliers affect the regression (need update)
* Conclusion: need update the code and results 

```{r, results='show', message=TRUE, include=TRUE, echo=FALSE, warning=FALSE, fig.height = 5, fig.width = 7}
srdb_01 <- filtration1(srdb_orig)
figB_01 <- Rs_comparion_figure( srdb_01,'3.4.1_Test outlier', "Test whether outliers affect Rs_Annual vs. Rs_annual_bahn" )
test_extreme <- Rs_annual_bahn_test(srdb_01, srdb_01$Rs_annual_bahn, var_title = 'Residual plot of outlier testing')
...
```{r, results='show', message=TRUE, include=TRUE, echo=FALSE, fig.width=5, fig.height= 9}
results <- data.frame()
...
```{r, , results='show', message=TRUE, include=TRUE, warning=FALSE, echo=FALSE, fig.height = 5, fig.width = 7}
srdb_03 <- filtration3( srdb_orig, 1 )
figB_03 <- Rs_comparion_figure( srdb_03,'3.4.2_TAIR_LTM_dev set to 1', 'Plot the results when difference between TAIR \nfrom paper and TAIR from Delaware > 1' )
test_TS <- Rs_annual_bahn_test(srdb_03, srdb_03$Rs_annual_bahn, var_title = 'Residual plot of TAIR difference testing')
m_TS <- test_TS[[ 1 ]]
m1_TS <- test_TS[[ 2 ]]
...
```{r, results='show', message=TRUE, include=TRUE, echo=FALSE, fig.height = 5, fig.width = 7 }
# See how Rs_annual is related to Rs_annual_bahn_Temp
TSTSMAT_test(srdb, srdb$Rs_annual_bahn, panel_lab = '(a) TS as predictor', var_title = 'Rs_Annual vs. Rs_annual_bahn \nwhen using mean annual Ts predict Rs_MAT')
test_TS <- Rs_annual_bahn_test(srdb, srdb$Rs_annual_bahn, var_title = 'Residual plot of Rs_Annual vs. Rs_annual_bahn \nwhen using mean annual Ts predict Rs_MAT')
m_TS <- test_TS[[ 1 ]]
m1_TS <- test_TS[[ 2 ]]
intercept_test <- summary( m_TS )$coefficients[ 1, 4 ]
slope_test <- summary( m1_TS )$coefficients[ 2, 4 ]
...
```{r, results='show', message=TRUE, include=TRUE, echo=FALSE, warning=FALSE, fig.height = 5, fig.width = 7}
# TAnnual, function 
TSTSMAT_test(srdb, srdb$Rs_annual_bahn_TAnnual, panel_lab = '(b) T_Annual as predictor', 'Rs_Annual vs. Rs_annual_bahn \nwhen using mean annual TAIR predict Rs_MAT')
test_TAnnual <- Rs_annual_bahn_test(srdb, srdb$Rs_annual_bahn_TAnnual, var_title = 'Residual plot')
...
```{r, results='show', message=TRUE, include=TRUE, warning=FALSE, echo=FALSE, fig.height = 5, fig.width = 7}
# MAT, function 
TSTSMAT_test(srdb, srdb$Rs_annual_bahn_MAT, panel_lab = '(c) MAT as predictor', 'Rs_Annual vs. Rs_annual_bahn \nwhen using mean annual Ts predict Rs_MAT')
test_MAT <- Rs_annual_bahn_test(srdb, srdb$Rs_annual_bahn_MAT, var_title = 'Residual plot')
m_MAT <- test_MAT[[ 1 ]]
m1_MAT <- test_MAT[[ 2 ]]
...
```{r filtration_mk, message=TRUE, include=TRUE, echo=FALSE, fig.height = 8, fig.width = 5}
...
```{r agriculture, results='show', message=TRUE, include=TRUE, warning=FALSE, echo=FALSE, fig.height = 5, fig.width = 7}
...
test_mv_ag <- Rs_annual_bahn_test(srdb_mv_ag, srdb_mv_ag$Rs_annual_bahn, var_title = 'Residual plot')
...
test_kp_ag <- Rs_annual_bahn_test(srdb_kp_ag, srdb_kp_ag$Rs_annual_bahn, var_title = 'Residual plot for when only agriculture data used')
...
```{r wetland, results='show', message=TRUE, include=TRUE, warning=FALSE, echo=FALSE, fig.height = 5, fig.width = 7}
...
test_mv_wet <- Rs_annual_bahn_test(srdb_mv_wet, srdb_mv_wet$Rs_annual_bahn, var_title = 'Residual plot when data from wetland were removed')
...
test_kp_wet <- Rs_annual_bahn_test(srdb_kp_wet, srdb_kp_wet$Rs_annual_bahn, var_title = 'Residual plot when only data from wetland were used')
...
figB_mv_IRGA <- Rs_comparion_figure( srdb_mv_IRGA, var_title = 'Rs measured by Alkali, soda line and gradient' )
test_mv_IRGA <- Rs_annual_bahn_test(srdb_mv_IRGA, srdb_mv_IRGA$Rs_annual_bahn, var_title = 'Residual plot for Alkali')
...
figB_kp_IRGA <- Rs_comparion_figure( srdb_kp_IRGA, var_title = 'Rs measured by IGRA and Gas Chromatography' )
test_kp_IRGA <- Rs_annual_bahn_test(srdb_kp_IRGA, srdb_kp_IRGA$Rs_annual_bahn, var_title = 'Residual plot for Rs measured by IGRA and Gas Chromatography')
...
```{r, results='show', message=TRUE, include=TRUE, echo=FALSE, fig.width=5, fig.height = 8}
...
climate_variability_test( srdb, var_title_T = 'Test TAIR variability and its effect on Rs_Annual vs. Rs_annual_bahn'
                          , var_title_P = 'Test precipitation variability and its effect on Rs_Annual vs. Rs_annual_bahn' )
```

## 4.2.7 Does drought affect?
```{r, results='show', message=TRUE, include=TRUE,warning=FALSE , echo=FALSE, fig.height = 5, fig.width = 7}
climate_MAP_test (srdb, 'Test how MAP affects the relationship between\n Rs_Annual and Rs_annual_bahn')
```

```{r, results='show', message=TRUE, include=TRUE, echo=FALSE, fig.height = 5, fig.width = 7}
# Do drought effects different trends?
SPI_test( srdb, 'Test how SPI affects the relationship between\n Rs_Annual and Rs_annual_bahn' ) #SPI can identify the drout of a year compare to the past 50 years (1964-2014)
```


```{r, results='show', message=TRUE, include=TRUE, echo=FALSE, fig.height = 5, fig.width = 7}
# Do drought effects different trends?
PDSI_test( srdb, 'Test how PDSI affects the relationship between\n Rs_Annual and Rs_annual_bahn' ) # need a new SPI data
```
