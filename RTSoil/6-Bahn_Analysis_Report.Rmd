...
# Adjust amat by 2.918 + 0.928*amat
...
```{r, include=FALSE}
obs_srdb <- nrow(srdb)
obs_med <- nrow(subset(srdb,Biome == 'Mediterranean' ))
obs_others <- nrow(subset(srdb,Biome != 'Mediterranean' ))
n_study <- length(unique(srdb$Study_number))
```

**Problem**
Robustly scaling soil respiration (Rs) across time and space is important to constrain and understand global Rs, a large C flux from terrestrial to atmosphere. However, Rs is difficult to measure continuously. It is common to use Rs measured at a single point in time to represent the average of longer time periods, e.g. daily Rs or monthly Rs. Due to the non-linear relationship between Rs and temperature, however, Rs at mean temperature cannot directly represent annual soil respiration. To resolve this issue, Bahn et al. (2010) developed a model to estimate annual Rs from Rs measured at annual mean soil temperature (amst). The Bahn(2010) model was based on 80 sites worldwide, but the robustness of this approach has not been evaluated in different biomes, ecosystems, and carbon cycle conditions. 

**Goals**
We evaluated the Bahn(2010) model using data from SRDB_V4 (`r obs_srdb`  records, an order of magnitude more data than Bahn et al. (2010)). Since long term and high spatial resolution soil temperature data are rare, we also tested whether Rs measured at annual mean _air_ temperature (Rs_amat), or mean air temperature across 1964-2017 (Rs_mat), can predict Rs_annual robustly.

**Main findings**
* We found that Rs_annual_bahn [i.e., Rs estimated using the original Bahn(2010) model] does not well represent Rs_annual as reported in the SRDB (slope=0.75, p<0.001, intercept=164, p<0.001).
* We examined the effects of soil temperature (Ts) sources, annual Rs or Ts coverage, cliamte data bias or variability, ecosystem type, biome, Rs measurement method, heterotrophic- vs. autotrophic-dominated sites, and drought. We found that biome, ecosystem type, Ra-or-Rh dominated sites, and drought have a significant affect on Rs_annual_bahn vs. Rs_annual relationship.
* We re-parameterized the Bahn(2010) model (i.e., with new parameters but the same equation, `new_model1`), and also build a model with SPI and PDSI included (new_model2). Rs_annual_bahn matched Rs_annual well under `new_model1`, but the residuals showed an increasing trend with SPI and PDSI, implying that the `new_model` increasingly overpredicted Rs as drought conditions increased.
* This trend disappered when SPI and PDSI were included (`new_model2`). Rs_annual can also be well predicted using Rs_amat or Rs_mat, however, a new model is required.
* The Bahn(2010) model, which originally used 80 sites across globel, needs to be re-parameterized before it can be applied at global scales. We found that SPI and PDSI are good indicators to resolve the drought issue--i.e. predict annual Rs with little or no bias. 
...
![Rs_mat vs Rs_annual (A) and Rs_annual_bahn vs Rs_annual (B)](SI/3. Rs_MAT_Rs_Annual_Bahn2010.jpeg)


# The objects of this analysis are

* To examine whether Rs_amst can represent Rs_annual using data from SRDB_V4. We have `r obs_srdb` records, an order of magnitude more data than Bahn et al. (2010).
* Since long term and high spatial resolution soil temperature data are rare, test whether Rs measured at annual mean _air_ temperature (Rs_amat), or mean air temperature across 1964-2014 (Rs_mat), can predict Rs_annual robustly.
...
* `r obs_srdb` records from `r n_study` studies.
* Annual mean soil temperature (reported in the papers or can be calculated with simple assumptions).
* Air temperature (University of Daleware global precipitation and air temperature data, 1964-2017, half degree spatial resolution).
* Data used in this study have a reasonable MAT and MAP coverage comparing with global MAT and MAP
...
* According the the relationship between Rs and Ts, we can estimate `Rs_amst/Rs_amat/Rs_mat` based on the amst, amat, and/or mat.
* Use the Bahn model to predict `Rs_annual` based on `Rs_amst/Rs_amat/Rs_mat`.
...
```{r 3.2 Rs or Ts coverage and effect}
...
```{r 3.3 test divergence, fig.height= 4, fig.width=8, include=FALSE}
results <- data.frame()
...
```{r 3.4 test precipitation divergence, fig.height=4, fig.width=8, include=FALSE}
results <- data.frame()
...
We tested the effect of precipitation and temperature variability [quantified by the standard deviation of annual mean air temperature (amat) between 1964 and 2017, and annual precipitation deviation between 1964 and 2017], using multiple linear regression, with divergence as catergorical indicator.
...
```{r filtration_mk, fig.height = 4, fig.width = 8, include=FALSE}
...
```{r wetland, warning=FALSE, include=FALSE}
...
* We thus updated the Bahn (2010) model's parameters (but with same formulation, named `new_model1` model).
* Following Bahn (2010), and because of the test performed above, we built a model for Mediterranean (n=`r obs_med`), and another model for the rest of the data (n=`r obs_others`). 


```{r new_model1, message=FALSE, fig.height=4, fig.width=8}
...
* We thus parameterized another model (`new_model2`, including SPI and PDSI as predictors).

```{r new_model2, message=FALSE, fig.width=9, fig.height=3.5}
...
* Comparing results from Bahn (2010), `new_model1`, and `new_model2` models:

```{r, warning=FALSE, fig.width=8, fig.height=8}
bahn2010 <- bahn_vs_new (srdb_orig, 'Rs_annual_bahn', var_title = 'Bahn (2010)', res_title = 'Residual plot')
bahn_new1 <- bahn_vs_new (srdb_orig, 'Rs_annual_bahn_new1', var_title = 'new_model1', res_title = 'Residual plot')
bahn_new2 <- bahn_vs_new (srdb_orig, 'Rs_annual_bahn_new2', var_title = 'new_model2', res_title = 'Residual plot')

print(plot_grid(bahn2010, bahn_new1, bahn_new2, nrow = 3))
```

* Comparing Bahn(2010), the new_model1 and new_model2 improved the model performance (Rs_annual_bahn vs Rs_annual regression more colser to 1:1).
...
```{r, warning=FALSE, fig.width=8, fig.height=5, warning=FALSE}
# TAnnual, function 
bahn_new2_TAnnual <- bahn_vs_new (srdb_orig, 'Rs_annual_bahn_TAnnual', var_title = 'Bahn (new_model1)', res_title = 'Residual plot')
bahn_new2_MAT <- bahn_vs_new (srdb_orig, 'Rs_annual_bahn_MAT', var_title = 'new_model2', res_title = 'Residual plot')
...
bahn_new2_TAnnual_adj <- bahn_vs_new (srdb_orig, 'Rs_annual_bahn_TAnnual_adj', var_title = 'new_model1', res_title = 'Residual plot')
bahn_new2_MAT_adj <- bahn_vs_new (srdb_orig, 'Rs_annual_bahn_MAT_adj', var_title = 'new_model2', res_title = 'Residual plot')
...
```{r}
model_mat <- rbind( Rs_lm_mat(srdb, 'Rs_annual_bahn', 'Bahn(2010)')
      , Rs_lm_mat(srdb, 'Rs_annual_bahn_new1', 'new_model1')
      , Rs_lm_mat(srdb, 'Rs_annual_bahn_new2', 'new_model2')
      , Rs_lm_mat(srdb, 'Rs_annual_bahn_TAnnual_maat', 'new_model3')
      )
```


```{r, results="asis"}
knitr::kable(model_mat, caption = "Model parameters and statistic summary", floating.environment="sidewaystable")
```


# 5. Discussion & questions

* Rs_amst vs Rs_annual, the red dots are measurements from Mediterranean, the blue dots are measurements from other biomes.
* Red and blue dashed lines are Bhan (2010) models for Mediterranean and exclude-Mediterranean, respectively.
* Red and blue solid lines are new_model1 for Mediterranean and exclude-Mediterranean, respectively.

```{r}
# plot and compare Bahn (2010) model vs new_model1
...
```{r, warning=FALSE, fig.width=8, fig.height=8, warning=FALSE}
bahn2010 <- bahn_vs_new (srdb_orig, 'Rs_annual_bahn', var_title = 'Bahn (2010)', res_title = 'Residual plot')
bahn_new1 <- bahn_vs_new (srdb_orig, 'Rs_annual_bahn_new1', var_title = 'new_model1', res_title = 'Residual plot')
bahn_new2 <- bahn_vs_new (srdb_orig, 'Rs_annual_bahn_new2', var_title = 'new_model2', res_title = 'Residual plot')
...
bahn_new2_MAT <- bahn_vs_new (srdb_orig, 'Rs_annual_bahn_MAT', var_title = 'new_model2', res_title = 'Residual plot')
bahn_new2_maat_MAT <- bahn_vs_new (srdb_orig, 'Rs_annual_bahn_MAT_maat', var_title = 'new_model3', res_title = 'Residual plot')
